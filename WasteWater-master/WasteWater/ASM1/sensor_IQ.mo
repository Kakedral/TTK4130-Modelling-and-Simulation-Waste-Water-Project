...
model sensor_IQ "Ideal sensor to measure chemical oxygen demand (COD)"
...
equation
  ...
  BOD = 0.65*(In.Ss + In.Xs + (1 - 0.08)*(In.Xbh + In.Xba));
  SS = 0.75*(In.Xs + In.Xi + In.Xbh + In.Xba + In.Xp);
  Snkj = In.Snh + In.Snd + In.Xnd + 0.008*(In.Xbh + In.Xba) + 0.06*(In.Xp + In.Xi);
  der(T) = 1.0;
  der(IQ) = -1/T*IQ + 1/(1000*T) * (2*SS + 1*COD + 30*Snkj + 10*In.Sno + 2*BOD)*18061;
